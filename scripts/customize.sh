...
if [ $INS = true ]; then
	if [ ! -f $MODPATH/__PKGNAME.apk ]; then
		abort "ERROR: No se encontró __PKGNAME apk stock"
	fi
	...
	if BASEPATH=$(pm path __PKGNAME); then
		...
		abort "ERROR: instalá __PKGNAME manualmente y reflashea el módulo"
	fi
fi
...
ui_print "* Terminado"
ui_print "  Fork de XxFrAnCoGoLxX (github.com/XxFrAnCoGoLxX) y hecho por j-hc (github.com/j-hc)"
ui_print " "
